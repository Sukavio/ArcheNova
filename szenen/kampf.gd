...
func _ready():
	...
	if spieler.speed > gegner.speed:
		...
		starter = gegner.monster
		status = FightStatus.Enemy
	%Dialog.text = str('Der Kampf zwischen ', spieler.name, ' und ', gegner.monster, ' beginnt!\n', starter, ' beginnt den Kampf.')
...
func _on_dice_tray_results_ready(moveNr: int):
	match status:
		FightStatus.Player:
			...
			openDialog = true
			%Dialog.text = str(spieler.name, ' greift ', gegner.monster, ' an und macht ', damage, ' Schaden!')
			if gegner.hp_value <= 0:
				%Dialog.text += str('\n', gegner.monster, " ist gestorben!")
				gegner.queue_free()
		...
		FightStatus.Enemy:
			...
			%Dialog.text = str(gegner.monster, ' greift ', spieler.name, ' an und macht ', damage, ' Schaden!')
